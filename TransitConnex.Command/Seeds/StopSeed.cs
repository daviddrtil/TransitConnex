...
public class StopSeed
{
    public static void Seed(AppDbContext context)
    {
        ...
        context.Stops.AddRange(stopsToBeSeeded);
        context.SaveChanges();
        ...
        {
            var locationStops = stopsToBeSeeded.Where(x => x.Name.Equals(location.Name)).ToList();
        ...
        foreach (var location in locations) // seeding relations for cities
        {
            ...
            {
                var locationStops = stopsToBeSeeded.Where(x => x.StopType != StopTypeEnum.TRAIN || x.Name == "Brno hln.").ToList();
                locationStopsToBeSeeded.AddRange(locationStops.Select(locationStop => new LocationStop() {LocationId = location.Id, StopId = locationStop.Id,}));
            }
            else
            {
                var locationStops = stopsToBeSeeded.Where(x => x.StopType == StopTypeEnum.TRAIN && x.Name != "Brno hln." && x.Name.Contains(location.Name)).ToList();
                locationStopsToBeSeeded.AddRange(locationStops.Select(locationStop => new LocationStop() {LocationId = location.Id, StopId = locationStop.Id,}));
            }
        }
